...
async function getModelos(){ if(cache.modelos && Date.now()-cache.mtime<CACHE_MS) return cache.modelos; try{ let snap=await getDocs(collection(db,"modelos")); cache.modelos=snap; cache.mtime=Date.now(); return snap; }catch(e){ return cache.modelos||{size:0,docs:[],forEach:()=>{}}; } }
...
// 13. TU CODIGO PREMIUM INTEGRADO - ADAPTADO A TELEGRAF
    ...
    if (msg && msg.text && msg.entities) {
      for (const entity of msg.entities) {
        if (entity.type === 'custom_emoji') {
          ...
          // Solo si es admin en modo edición, confirma ID y devuelve premium
          const key = String(ctx.from?.id||"");
          if (esperando[key] && await isAdmin(ctx)) {
            // OPCIÓN A: Responder confirmando que leíste el ID
            await ctx.reply(`He leído tu emoji premium. ID: ${emojiId}`, { parse_mode: 'HTML' }).catch(()=>{});
            // OPCIÓN B: Replicar usando <tg-emoji>
            await ctx.reply(`Aquí tienes tu emoji de vuelta: <tg-emoji emoji-id="${emojiId}">${emojiPlano}</tg-emoji>`, { parse_mode: 'HTML' }).catch(()=>{});
          }
        }
      }
    }
  } catch (e) { console.log("Error premium:", e.message); }
  if (next) return next();
});

const PANEL_TEXTO=`👑 <b>PANEL ADMIN V13</b> 👑`;
...
bot.action('panel_bienvenida',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let c=await getConfig(); await ctx.reply(`👋🏻 BIENVENIDA\nFoto: ${c.bienvenida_media?'✅ Telegram':'❌'}\nPremium: ${c.bienvenida_entities?.length||0} emojis\n${(c.bienvenida_texto||'').slice(0,300)}`,{reply_markup:{inline_keyboard:[[{text:"📸 Foto Telegram",callback_data:"edit_bienvenida_foto", style:"primary"},{text:"📝 Texto Premium",callback_data:"edit_bienvenida_texto", style:"primary"}],[{text:"👁️ Preview",callback_data:"preview_start", style:"success"}],[{text:"⬅️ Volver",callback_data:"back_admin", style:"danger"}]]}}); });
bot.action('panel_galeria',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let c=await getConfig(); await ctx.reply(`🖼️ GALERIA\nFoto: ${c.galeria_media?'✅':'❌'}\nPremium: ${c.galeria_entities?.length||0}`,{reply_markup:{inline_keyboard:[[{text:"📸 Foto",callback_data:"edit_galeria_foto", style:"primary"},{text:"📝 Texto Premium",callback_data:"edit_galeria_texto", style:"primary"}],[{text:"⬅️ Volver",callback_data:"back_admin", style:"danger"}]]}}); });
bot.action('panel_plantillas',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let snap=await getDocs(collection(db,"plantillas")).catch(()=>({docs:[]})); let kb=[]; snap.docs.slice(0,15).forEach(d=>{ kb.push([{text:`📄 ${d.data().nombre}`,callback_data:`plantilla_use_${d.id}`, style:"primary"},{text:"🗑️",callback_data:`plantilla_del_${d.id}`, style:"danger"}]); }); kb.push([{text:"➕ CREAR PLANTILLA PREMIUM",callback_data:"edit_plantilla_texto", style:"success"}]); kb.push([{text:"⬅️ Volver",callback_data:"back_admin", style:"danger"}]); await ctx.reply("📝 PLANTILLAS - Ilimitadas Telegram Cloud",{reply_markup:{inline_keyboard:kb}}); });
...
bot.action('panel_botones',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); await ctx.reply("🧩 PERSONALIZAR BOTONES - Nombre, color 🔴🔵🟢 y premium",{reply_markup:{inline_keyboard:[[{text:"BIENVENIDA",callback_data:"botones_bienvenida", style:"danger"}],[{text:"GALERIA",callback_data:"botones_galeria", style:"primary"}],[{text:"PLANTILLA",callback_data:"botones_plantilla", style:"success"}],[{text:"⬅️ Volver",callback_data:"back_admin", style:"danger"}]]}}); });

bot.action('botones_bienvenida',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let b=(await getConfig()).botones.bienvenida; await ctx.reply(`BIENVENIDA:\n1. [🔵 ${b.galeria_virtual.style}] ${b.galeria_virtual.text} WEDAPP\n2. [🔴 ${b.lista_modelos.style}] ${b.lista_modelos.text}\n3. [🟢 ${b.canal_oficial.style}] ${b.canal_oficial.text}`,{reply_markup:{inline_keyboard:[[{text:"1. Editar GALERÍA VIRTUAL 🔵",callback_data:"edit_btn_bienvenida_galeria_virtual", style:"primary"}],[{text:"2. Editar LISTA MODELOS 🔴",callback_data:"edit_btn_bienvenida_lista_modelos", style:"danger"}],[{text:"3. Editar CANAL 🟢",callback_data:"edit_btn_bienvenida_canal_oficial", style:"success"}],[{text:"⬅️ Volver",callback_data:"panel_botones", style:"danger"}]]}}); });
bot.action('botones_galeria',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let c=await getConfig(); await ctx.reply(`GALERIA:\nHilera1 ${c.botones.galeria.modelo_color_1} ${c.botones.galeria.modelo_style_1} | Hilera2 ${c.botones.galeria.modelo_color_2} ${c.botones.galeria.modelo_style_2}\nCanal ${c.botones.galeria.canal_oficial.style} Virtual ${c.botones.galeria.galeria_virtual.style}`,{reply_markup:{inline_keyboard:[[{text:"🎨 Hileras 🔵🔴",callback_data:"edit_galeria_colores", style:"primary"}],[{text:"CANAL 🟢",callback_data:"edit_btn_galeria_canal", style:"success"}],[{text:"VIRTUAL 🔵",callback_data:"edit_btn_galeria_virtual", style:"primary"}],[{text:"⬅️ Volver",callback_data:"panel_botones", style:"danger"}]]}}); });
bot.action('botones_plantilla',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let b=(await getConfig()).botones.plantilla; await ctx.reply(`PLANTILLA:\n[${b.perfil_completo.style}] ${b.perfil_completo.text} 🔵\n[${b.bueno.style}] ${b.bueno.text} 🟢 | [${b.malo.style}] ${b.malo.text} 🔴\n[${b.canal_free.style}] ${b.canal_free.text} [${b.contactame.style}] ${b.contactame.text} 🔵\n[${b.volver.style}] ${b.volver.text} [${b.inicio.style}] ${b.inicio.text} 🔴`,{reply_markup:{inline_keyboard:[[{text:"PERFIL 🔵",callback_data:"edit_btn_plantilla_perfil", style:"primary"}],[{text:"BUENO 🟢",callback_data:"edit_btn_plantilla_bueno", style:"success"},{text:"MALO 🔴",callback_data:"edit_btn_plantilla_malo", style:"danger"}],[{text:"FREE 🔵",callback_data:"edit_btn_plantilla_free", style:"primary"},{text:"CONTACTAME 🔵",callback_data:"edit_btn_plantilla_contacto", style:"primary"}],[{text:"VOLVER 🔴",callback_data:"edit_btn_plantilla_volver", style:"danger"},{text:"INICIO 🔴",callback_data:"edit_btn_plantilla_inicio", style:"danger"}],[{text:"⬅️ Volver",callback_data:"panel_botones", style:"danger"}]]}}); });

bot.action(/edit_btn_(.*)/,async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); esperando[String(ctx.from.id)]=`btn_${ctx.match[1]}`; await ctx.reply(`🧩 Manda NUEVO NOMBRE con premium para ${ctx.match[1]}\nDetectaré ID con <tg-emoji>`); });
bot.action('edit_galeria_colores',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); esperando[String(ctx.from.id)]='galeria_colores'; await ctx.reply("Manda: 🔵 🔴"); });
bot.action('edit_bienvenida_foto',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); esperando[String(ctx.from.id)]='foto_bienvenida'; await ctx.reply("📸 Foto Telegram Cloud gratis"); });
bot.action('edit_bienvenida_texto',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); esperando[String(ctx.from.id)]='texto_bienvenida'; await ctx.reply("📝 Texto bienvenida premium + variables:\n{mencion} {perfil} @{username} {edad} {nacionalidad} {Lista_servicios} {descripcion} {Votos} {porcentaje_buenos}% {porcentaje_malos}%"); });
...
bot.action('edit_plantilla_texto',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); esperando[String(ctx.from.id)]='texto_plantilla'; await ctx.reply("📝 Plantilla premium + variables"); });
bot.action('admin_add',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); esperando[String(ctx.from.id)]='add_admin_id'; await ctx.reply("ID admin"); });
...
bot.on('photo',async(ctx)=>{
  ...
  if(st.startsWith('foto_modelo_')){ let idModel=st.replace('foto_modelo_',''); await setDoc(doc(db,"modelos",idModel),{foto_file_id:fileId,foto:fileId},{merge:true}); clearCache(); delete esperando[key]; return ctx.reply("✅ Foto modelo Telegram Cloud"); }
...
bot.on('text',async(ctx)=>{
  ...
  if(st.startsWith('btn_')){
    ...
    return ctx.reply(`✅ Botón ${path} -> ${txt} con premium guardado`,{parse_mode:'HTML'});
  }
  if(st==='galeria_colores'){ let parts=txt.trim().split(/\s+/); let map={"🔵":"primary","🔴":"danger","🟢":"success"}; let s1=map[parts[0]]||"primary"; let s2=map[parts[1]]||"danger"; let c=await getConfig(); c.botones.galeria.modelo_style_1=s1; c.botones.galeria.modelo_style_2=s2; c.botones.galeria.modelo_color_1=parts[0]; c.botones.galeria.modelo_color_2=parts[1]; await setDoc(doc(db,"config","bot"),{botones:c.botones},{merge:true}); clearCache(); delete esperando[key]; return ctx.reply(`✅ Hileras: 1-${s1} 2-${s2}`); }
  if(st==='texto_bienvenida'){ await setDoc(doc(db,"config","bot"),{bienvenida_texto:txt,bienvenida_entities:entities},{merge:true}); clearCache(); delete esperando[key]; return ctx.reply("✅ Bienvenida premium con ID guardado"); }
...
bot.action('lista',async(ctx)=>{
  ...
  snap.forEach(d=>{
    ...
    row.push({text:d.data().perfil||d.id, callback_data:`ver_${d.id}`, style});
    if(row.length===2){ kb.push(row); row=[]; } i++;
  });
...
app.get('/',(req,res)=>res.send('Bot V13 FINAL 13 FUNCIONES OK'));
app.post('/webhook',(req,res)=>{ bot.handleUpdate(req.body).then(()=>res.send('ok')).catch(()=>res.send('ok')); });
const PORT=process.env.PORT||3000;
app.listen(PORT,async()=>{ console.log('Bot V13'); const ext=process.env.RENDER_EXTERNAL_URL; if(ext){ try{ await bot.telegram.setWebhook(`${ext}/webhook`); }catch(e){} } });
