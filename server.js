...
const DEFAULT_BOTONES = {
  bienvenida: {
    galeria_virtual: { text:"💖 VER GALERÍA VIRTUAL 💖", type:"webapp" },
    lista_modelos: { text:"👑 VER LISTA DE MODELOS 👑", type:"callback", data:"lista" },
    canal_oficial: { text:"💎 CANAL OFICIAL 💎", type:"url", url:CANAL_OFICIAL }
  },
  galeria: {
    modelo_color_1: "🔵", modelo_color_2: "🔴",
    canal_oficial: { text:"💎 CANAL OFICIAL 💎", url:CANAL_OFICIAL },
    galeria_virtual: { text:"💖 VER GALERÍA VIRTUAL 💖", type:"webapp" }
  },
  plantilla: {
    perfil_completo: { text:"💖 VER PERFIL COMPLETO 💖", type:"webapp" },
    bueno: { text:"👍 Bueno", type:"voto" },
    malo: { text:"👎 Malo", type:"voto" },
    canal_free: { text:"💎 CANAL FREE", type:"url" },
    contactame: { text:"💬 CONTACTAME", type:"url" },
    volver: { text:"👈 VOLVER", type:"callback", data:"lista" },
    inicio: { text:"👑 INICIO", type:"callback", data:"inicio" }
  }
};
...
// VARIABLES NUEVAS EXACTAS COMO PIDES
function replaceVars(str,m={},ctx=null){
  ...
  return str
   .replaceAll('{mencion}', mencion).replaceAll('{Mencion}', mencion)
   .replaceAll('{perfil}', m.perfil||'').replaceAll('{Perfil}', m.perfil||'')
   .replaceAll('@{username}', m.username?`@${m.username}`:'').replaceAll('{username}', m.username||'')
   .replaceAll('{edad}', String(m.edad||'')).replaceAll('{Edad}', String(m.edad||''))
   .replaceAll('{nacionalidad}', m.nacionalidad||'').replaceAll('{Nacionalidad}', m.nacionalidad||'')
   .replaceAll('{Lista_servicios}', Lista_servicios).replaceAll('{lista_servicios}', Lista_servicios).replaceAll('{LISTA_SERVICIOS}', Lista_servicios).replaceAll('{servicios}', Lista_servicios)
   .replaceAll('{descripcion}', m.descripcion||'').replaceAll('{Descripcion}', m.descripcion||'')
   .replaceAll('{Votos}', String(total)).replaceAll('{votos}', String(total))
   .replaceAll('{porcentaje_buenos}%', `${pBueno}%`).replaceAll('{porcentaje_buenos}', String(pBueno)).replaceAll('{porcentajeBuenos}%', `${pBueno}%`).replaceAll('{porcentajeBuenos}', String(pBueno)).replaceAll('{porcentajeBueno}%', `${pBueno}%`).replaceAll('{porcentajeBueno}', String(pBueno))
   .replaceAll('{porcentaje_malos}%', `${pMalo}%`).replaceAll('{porcentaje_malos}', String(pMalo)).replaceAll('{porcentajeMalos}%', `${pMalo}%`).replaceAll('{porcentajeMalos}', String(pMalo)).replaceAll('{porcentajeMalo}%', `${pMalo}%`).replaceAll('{porcentajeMalo}', String(pMalo))
   .replaceAll('{id}', m.id||'').replaceAll('{canalFree}', m.canalFree||'').replaceAll('{contacto}', m.contacto||'');
}

let esperando={}; let temp={};
const PANEL_TEXTO=`👑 <b>PANEL ADMIN</b> 👑\nTelegram Cloud + Premium`;
...
bot.command('admin',async(ctx)=>{ if(!(await isAdmin(ctx))) return ctx.reply("❌"); delete esperando[String(ctx.from.id)]; await ctx.reply(PANEL_TEXTO,{parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:"👋 BIENVENIDA",callback_data:"panel_bienvenida"},{text:"📝 PLANTILLAS",callback_data:"panel_plantillas"}],[{text:"🖼️ GALERIA",callback_data:"panel_galeria"},{text:"👸 MODELOS",callback_data:"panel_modelos"}],[{text:"👑 ADMINS",callback_data:"panel_admins"},{text:"🧩 BOTONES",callback_data:"panel_botones"}]]}}); });
bot.command('cancel',async(ctx)=>{ delete esperando[String(ctx.from.id)]; await ctx.reply("✅ Cancelado"); });

bot.action('panel_bienvenida',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let c=await getConfig(); await ctx.reply(`👋 BIENVENIDA\nFoto: ${c.bienvenida_media?'✅':'❌'}\nTexto: ${(c.bienvenida_texto||'').slice(0,200)}`,{reply_markup:{inline_keyboard:[[{text:"📸 Foto",callback_data:"edit_bienvenida_foto"},{text:"📝 Texto Premium",callback_data:"edit_bienvenida_texto"}],[{text:"👁️ Preview",callback_data:"preview_start"}],[{text:"⬅️ Volver",callback_data:"back_admin"}]]}}); });
...
bot.action('panel_plantillas',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let snap=await getDocs(collection(db,"plantillas")).catch(()=>({docs:[]})); let kb=[]; snap.docs.slice(0,15).forEach(d=>{ kb.push([{text:`📄 ${d.data().nombre}`,callback_data:`plantilla_use_${d.id}`},{text:"🗑️",callback_data:`plantilla_del_${d.id}`}]); }); kb.push([{text:"➕ CREAR PREMIUM",callback_data:"edit_plantilla_texto"}]); kb.push([{text:"⬅️ Volver",callback_data:"back_admin"}]); await ctx.reply("📝 PLANTILLAS",{reply_markup:{inline_keyboard:kb}}); });
bot.action('panel_modelos',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let snap=await getModelos(); let kb=[]; let row=[]; snap.forEach(d=>{ row.push({text:d.data().perfil||d.id,callback_data:`mfoto_${d.id}`}); if(row.length===2){ kb.push(row); row=[]; } }); if(row.length) kb.push(row); kb.push([{text:"⬅️ Volver",callback_data:"back_admin"}]); await ctx.reply(`👸 MODELOS (${snap.size})`,{reply_markup:{inline_keyboard:kb}}); });
...
bot.action('panel_botones',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); await ctx.reply("🧩 BOTONES PREMIUM",{reply_markup:{inline_keyboard:[[{text:"BIENVENIDA",callback_data:"botones_bienvenida"}],[{text:"GALERIA",callback_data:"botones_galeria"}],[{text:"PLANTILLA",callback_data:"botones_plantilla"}],[{text:"⬅️ Volver",callback_data:"back_admin"}]]}}); });
bot.action('botones_bienvenida',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let c=await getConfig(); let b=c.botones.bienvenida; await ctx.reply(`BIENVENIDA:\n1.${b.galeria_virtual.text}\n2.${b.lista_modelos.text}\n3.${b.canal_oficial.text}`,{reply_markup:{inline_keyboard:[[{text:"1. Editar",callback_data:"edit_btn_bienvenida_galeria_virtual"}],[{text:"2. Editar",callback_data:"edit_btn_bienvenida_lista_modelos"}],[{text:"3. Editar CANAL",callback_data:"edit_btn_bienvenida_canal_oficial"}],[{text:"⬅️ Volver",callback_data:"panel_botones"}]]}}); });
bot.action('botones_galeria',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let c=await getConfig(); await ctx.reply(`Colores: ${c.botones.galeria.modelo_color_1} ${c.botones.galeria.modelo_color_2}`,{reply_markup:{inline_keyboard:[[{text:"🎨 Cambiar colores",callback_data:"edit_galeria_colores"}],[{text:"Editar CANAL",callback_data:"edit_btn_galeria_canal"}],[{text:"Editar VIRTUAL",callback_data:"edit_btn_galeria_virtual"}],[{text:"⬅️ Volver",callback_data:"panel_botones"}]]}}); });
bot.action('botones_plantilla',async(ctx)=>{ await ctx.answerCbQuery().catch(()=>{}); let c=await getConfig(); let b=c.botones.plantilla; await ctx.reply(`${b.perfil_completo.text}\n${b.bueno.text}|${b.malo.text}`,{reply_markup:{inline_keyboard:[[{text:"PERFIL",callback_data:"edit_btn_plantilla_perfil"}],[{text:"BUENO",callback_data:"edit_btn_plantilla_bueno"},{text:"MALO",callback_data:"edit_btn_plantilla_malo"}],[{text:"FREE",callback_data:"edit_btn_plantilla_free"},{text:"CONTACTO",callback_data:"edit_btn_plantilla_contacto"}],[{text:"VOLVER",callback_data:"edit_btn_plantilla_volver"},{text:"INICIO",callback_data:"edit_btn_plantilla_inicio"}],[{text:"⬅️ Volver",callback_data:"panel_botones"}]]}}); });
...
bot.action(/ver_(.*)/,async(ctx)=>{
  ...
  let kb=[
    [{text:b.perfil_completo.text, web_app:{url:`${WEBAPP_URL}?m=${m.id}`}}],
    [{text:`${b.bueno.text} ${m.votosBueno||0}`,callback_data:`voto_bueno_${m.id}`},{text:`${b.malo.text} ${m.votosMalo||0}`,callback_data:`voto_malo_${m.id}`}],
    [{text:b.canal_free.text, url:m.canalFree||CANAL_OFICIAL},{text:b.contactame.text, url:m.contacto||`https://t.me/${m.username||''}`}],
    [{text:b.volver.text, callback_data:"lista"},{text:b.inicio.text, callback_data:"inicio"}]
  ];
  try{ await ctx.deleteMessage(); }catch(e){}
...
app.get('/',(req,res)=>res.send('Bot V11 - VARIABLES NUEVAS'));
app.post('/webhook',(req,res)=>{ bot.handleUpdate(req.body).then(()=>res.send('ok')).catch(()=>res.send('ok')); });
const PORT=process.env.PORT||3000;
app.listen(PORT,async()=>{ console.log('Bot V11'); const ext=process.env.RENDER_EXTERNAL_URL; if(ext){ try{ await bot.telegram.setWebhook(`${ext}/webhook`); }catch(e){} } });
